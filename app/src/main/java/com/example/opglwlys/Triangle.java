...
import static com.example.opglwlys.ShaderUtil.createProgram;
...
//                    0.5f,-0.5f,0,
    ...
    {
    	//加载顶点着色器的脚本内容
        mVertexShader=ShaderUtil.loadFromAssetsFile("lvertex.sh", mv.getResources());
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("lfrag.sh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
...
//        GLES30.glDrawArrays(GLES30.GL_TRIANGLE_FAN, 0, vCount);
    ...
    public void setTex(float[] coords) {
        final float UNIT_SIZE=0.15f;
        float texCoor[]=coords;
